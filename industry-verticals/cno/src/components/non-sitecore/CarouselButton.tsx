...
const CarouselButton = ({ direction = 'next', className = '', ...props }: CarouselButtonProps) => {
  return (
    <button
      className={`!text-foreground bg-background border-border hover:bg-background-surface z-10 size-12 content-center rounded-sm border shadow-none transition-colors ${className}`}
      {...props}
    >
