        ...
        {images.map((img, idx) => {
          ...
          return (
            <button
              ...
              className={`bg-background-muted focus:ring-interactive size-15 overflow-hidden rounded focus:ring-2 focus:outline-none xl:size-18 ${
                isActive ? 'cursor-not-allowed opacity-50' : 'hover:ring-interactive hover:ring-2'
              }`}
            >
      ...
      <div className="grow">
        {images[mainImageIndex] ? (
          <ContentSdkImage
            ...
            className="bg-background-muted aspect-square w-full rounded-sm object-contain p-4"
          />
        ) : (
          <div className="bg-background-muted aspect-square w-full rounded-sm" />
        )}
      </div>
    </div>
  );
