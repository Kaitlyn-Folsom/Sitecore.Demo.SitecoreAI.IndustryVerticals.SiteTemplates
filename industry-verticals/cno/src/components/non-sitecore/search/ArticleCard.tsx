...
const ArticleItemCard = ({ className = '', article }: ArticleItemCardProps) => {
  ...
  return (
    ...
    >
      <ArticleCard.Root
        key={article.id}
        className={`group border-border hover:shadow-interactive/10 relative border shadow-sm hover:shadow-sm hover:transition-colors hover:duration-200 hover:ease-linear ${className}`}
      >
        ...
        <div className="relative m-4 flex-col justify-between">
          ...
          <ArticleCard.Subtitle className="text-foreground-light mt-3 flex text-sm">
            <div className="text-foreground-muted group-hover:text-interactive right-0 flex items-center gap-1 text-sm font-medium transition-colors">
